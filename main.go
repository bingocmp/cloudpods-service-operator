...
func main() {
	...
	if err = (&controllers.GeneralServiceReconciler{
		ReconcilerBase: controllers.ReconcilerBase{
			Client: mgr.GetClient(),
			Log:    ctrl.Log.WithName("controllers").WithName("GeneralService"),
			Scheme: mgr.GetScheme(),
		},
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "GeneralService")
		os.Exit(1)
	}
